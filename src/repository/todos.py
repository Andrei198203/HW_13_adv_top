...
# async def create_todo(body=None):
...
async def create_todo(body: TodoSchema, db: AsyncSession, user: User):  # Исправлено тут
    todo = Todo(**body.dict(exclude_unset=True), user=user)  # Исправлено тут
    db.add(todo)
    await db.commit()
    await db.refresh(todo)
    return todo
