...
cloudinary.config(cloud_name=config.CLD_NAME, api_key=config.CLD_API_KEY, api_secret=config.CLD_API_SECRET,
                  secure=True)
...
async def get_current_user(file: UploadFile = File(), user: User = Depends(auth_service.get_current_user),
    ...
    res_url = cloudinary.CloudinaryImage(public_id).build_url(
        width=250, heigh=250, crop="fill", version=res.get("version"))
