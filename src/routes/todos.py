...
async def get_todos(limit: int = Query(default=10, ge=10, le=500), offset: int = Query(default=0, ge=0),
...
async def get_all_todos(limit: int = Query(default=10, ge=10, le=500), offset: int = Query(default=0, ge=0),
                    db: AsyncSession = Depends(get_db), user: User = Depends(auth_service.get_current_user)):
